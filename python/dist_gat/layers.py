    ...
    def forward(self, input, adj, nodes, epoch, weights, bias):
        ...
        if not context.glContext.config['isNeededExactBackProp']:
            ...
            needed_embs=context.glContext.dgnnClientRouterForCpp.getNeededEmb(
                ...
                context.glContext.config['bitNum'],context.glContext.config['isChangeRate'],store.isTrain, context.glContext.config['trend'],feat_size)
            ...
            #         try:
            #             # start=time.time()
            #             # t=threading.Thread(target=accessEmb,args=(q[i], i, epoch,self.layer_id,context.glContext.dgnnWorkerRouter[i],
            #             #                                             context.glContext.config['firstHopForWorkers'][i],context.glContext.config['id'],))
            #             # t.start()
            #             # _thread.start_new_thread(accessEmb, (needed_emb_map_from_workers, i, epoch,self.layer_id,))
            #             # p=Process(target=accessEmb,args=(q[i], i, epoch,self.layer_id,context.glContext.dgnnWorkerRouter[i],
            #             #                                  context.glContext.config['firstHopForWorkers'][i],context.glContext.config['id'],))
            #             # p.start()
            #             accessEmb(needed_emb_map_from_workers, i, epoch, self.layer_id)
            #             # end=time.time()
            #             # print("threadtime================={0}".format(end-start))
            #         except:
            #             print("Error:无法启动 process {0}".format(i))

            # while store.threadCountList[self.layer_id] != context.glContext.config['worker_num'] - 1:
            #     pass
            # store.threadCountList[self.layer_id] = 0

            end = time.time()

            print("pull emb {0} time {1}:".format(self.layer_id, (end - start)))
            # time_tmp=end-start
