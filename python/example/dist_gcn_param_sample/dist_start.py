...
import ecgraph.util_python.metric as metric
...
from ecgraph import optimizer as optim
...
from example.dist_gcn_param.models import GCN
import ecgraph.autograd.autograd_new as autoG
...
def updateParam():
    ...
    for i in range(layer_num):
        if i == 0:
            autograd.weight[i]=torch.FloatTensor(params['w' + str(i)]).reshape(feat_size, hidden[0])
            autograd.bias[i]=torch.FloatTensor(params['b' + str(i)]).reshape(hidden[0])

        elif i == layer_num - 1:
            autograd.weight[i]= torch.FloatTensor(params['w' + str(i)]).reshape(hidden[-1], class_num)
            autograd.bias[i]= torch.FloatTensor(params['b' + str(i)]).reshape(class_num)

        else:
            autograd.weight[i]= torch.FloatTensor(params['w' + str(i)]).reshape(hidden[i - 1], hidden[i])
            autograd.bias[i]= torch.FloatTensor(params['b' + str(i)]).reshape(hidden[i])
    ...
    for epoch in range(context.glContext.config['iterNum']):
        ...
        if epoch % context.glContext.config['print_result_interval'] == 0:
            ...
            print('Epoch: {:04d}'.format(epoch + 1),
                  ...
                  "epoch_iter_time: {:.4f}".format(
                      endTimeTotle - startTimeTotle - context.glContext.time_epoch['backward']),
