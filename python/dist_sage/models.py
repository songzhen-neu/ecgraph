...
from dist_sage.layers import SAGE_Layer
...
class GraphSAGE(nn.Module):
    def __init__(self, nfeat, nhid, nclass, dropout, autograd):  # 底层节点的参数，feature的个数；隐层节点个数；最终的分类数
        ...
        for i in range(1, nhid_len+2):
            if i == 1:
                self.gc.append(SAGE_Layer(nfeat, nhid[0], 0))
                ...
                self.gc.append(SAGE_Layer(nhid[i-2], nhid[i-1], i-1))
                ...
                self.gc.append(SAGE_Layer(nhid[i-2], nclass, i-1))
